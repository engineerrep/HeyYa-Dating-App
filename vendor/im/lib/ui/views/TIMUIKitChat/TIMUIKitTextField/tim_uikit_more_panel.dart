...
class MorePanel extends StatefulWidget {
  ...
  const MorePanel(
      ...
      : super(key: key);
  @override
  State<StatefulWidget> createState() => MorePanelState();
}

class MorePanelState extends TIMUIKitState<MorePanel> {
  ...
  static sendMediaMessage(BuildContext context,String conversationID, MorePanelState more) async {
    try {
        ...
        for (var asset in pickedAssets) {
          ...
          if (filePath != null) {
            if (type == AssetType.image) {
              final TUIChatViewModel m = serviceLocator<TUIChatViewModel>();
              MessageUtils.handleMessageError(
                  m.sendImageMessage(
            ...
            if (type == AssetType.video) {
              ...
              final size = await originFile!.length();
              if (size >= 104857600) {
                more.onTIMCallback(TIMCallback(
                    type: TIMCallbackType.INFO,
                    infoRecommendText: TIM_t("发送失败,视频不能大于100MB"),
                    infoCode: 6660405));
                return;
